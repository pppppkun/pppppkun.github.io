...
我们会发现这里的 $\Sigma$ 是一个奇异矩阵。这也就意味着其逆矩阵 $\Sigma^{-1}$ 不存在，而 $1/|\Sigma|^{1/2} = 1/0$。 但这几个变量都还是需要的，要用来计算一个多元高斯分布的常规密度函数。还可以用另外一种方法来讲述清楚这个难题，也就是对参数的最大似然估计会产生一个高斯分布，其概率分布在由样本数据$^1$所张成的仿射空间中，对应着一个奇异的协方差矩阵。
